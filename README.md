...
5. **🎖️ 音乐人黑胶会员进阶任务 (`ncmm musician`)**
   - 自动查询进阶任务进度，按需执行并加速任务达标：
     - **日常签到**：自动完成音乐人日常签到（申领云豆奖励）。
...
# 音乐人任务配置
musician:
  enableMain: true          # 是否启用主账号执行音乐人任务
  enableSecondaries: true   # 是否启用所有辅助账号执行音乐人任务
  identityCacheDays: 0      # 音乐人身份状态的本地缓存时间（单位：天），默认永久有效
