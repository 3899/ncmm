...
`--listen 0.0.0.0:3899 --token xxx` #允许其他机器访问时必须预先配置管理令牌；默认仅监听 `127.0.0.1:3899`

`--token xxx` # WebUI 登录令牌；本机首次打开可安全设置，非回环监听默认禁止无凭据启动

`--allow-remote-setup` #仅用于明确允许远程首次设置，设置时仍需输入启动日志中的一次性设置码
