...
11. **🖥️ 可选 WebUI (`ncmm web`)**：可视化编辑 `config.yaml` / `notify.yaml`，支持 Cookie 和二维码登录、管理定时任务与运行日志；管理端使用 PBKDF2 管理员密码、HttpOnly Session、CSRF 防护和登录限速，Docker 镜像默认在 `3899` 端口启用。
