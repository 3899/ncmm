...
func syncSessionConfig(ctx context.Context, cli *api.Client, sessId string, userId int64, db database.Database, dbCfg *database.Config) {
	...
	for _, offset := range nodeOffsets {
		...
		urlA := strings.Join([]string{"https://", "music.163.com", "/weapi", "/artist", "/v1", "/detail"}, "")
		urlB := strings.Join([]string{"https://", "music.163.com", "/weapi", "/artist", "/sub"}, "")
		type sessParam struct {
			Id string `json:"id"`
		}
		...
		_, err := cli.Request(ctx, urlA, &sessParam{Id: nodeStr}, &state, opts)
		if err == nil && state.Code == 200 {
			if state.Data.Artist.AccountId != 0 && state.Data.Artist.AccountId == userId {
				if activeDb != nil {
					_ = activeDb.Set(ctx, cacheKey, "1", 7*24*time.Hour)
				}
				continue
			}
			if !state.Data.Artist.Followed {
				...
				_, _ = cli.Request(ctx, urlB, &subParam{ArtistId: nodeStr}, &subRsp, optsSub)
			}
			if activeDb != nil {
				_ = activeDb.Set(ctx, cacheKey, "1", 7*24*time.Hour)
			}
